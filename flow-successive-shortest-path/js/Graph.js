...
Graph.prototype.removeEdge = function(id){
    ...
    var end = edge.end;
    start.outEdges.remove(id);
    end.inEdges.remove(id);
...
Graph.parse = function(text){
  ...
  function parseResourcesNodes(s){
      var resources = [];
      //for(var i=3; i<s.length-1; i++){
          resources.push(+s[3]);
     // }
  ...
  for (var line in lines) {
      ...
      if (s[0] == "n") {
          ...
          graph.addNode(s[1], s[2],parseResourcesNodes(s));
          continue;
		  }else{
			graph.addNode(s[1], s[2],[]);
			continue;
		  }
      }
      //s t r1 r2 ... 
      if (s[0] == "e") {
          ...
          graph.addEdge(s[1], s[2], resources, s[5]);    
      }
  
	  if(s[0] == "f"){
		graph.actualFlow = s[1];


	  };
	  if(s[0] == "c"){
		  graph.minimalCost = s[1];
	  };
...
Graph.setGraph = function(tabprefix) {
    ...
    svg.select("#graph1").remove();

    if (selection == "graph1") selection = "graph1";
    else if (selection == "graph2") selection = "graph2";
    else if (selection == "graph3") selection = "graph3";
    else if (selection == "graph4") selection = "graph4";
	else if (selection == "graph5") selection = "graph5";
	else if (selection == "graph6") selection = "graph6";
	else if (selection == "graph7") selection = "graph7";
	else if (selection == "graph8") selection = "graph8";
	else if (selection == "graph9") selection = "graph9";
