...
function parallelOrDoubleEdgeExistence(start_node, end_node){
  var start_node_edges = start_node.getInEdges();
  var end_node_edges = end_node.getInEdges();

  for(var i = 0; i<start_node_edges.length; i++){
    if(start_node_edges[i].start == end_node){
      return true;
    }
  }
  for(var i = 0; i<end_node_edges.length; i++){
    if(end_node_edges[i].start == start_node){
      return true;
    }
  }
  return false;
...
//       graph.addEdge(selectedNode.id,d.id);
    if(parallelOrDoubleEdgeExistence(d,selectedNode)){
      if(unfinishedEdge) Graph.instance.removeEdge(unfinishedEdge.id);
        deselectNode();
      ...
      unfinishedEdge.start.outEdges.set(unfinishedEdge.id,unfinishedEdge);
      unfinishedEdge.end.inEdges.set(unfinishedEdge.id,unfinishedEdge);
      deselectNode();
      //that.updateEdges();
    }
  }
...
//       .style("opacity", 1)
  ...
  .style("top", function(a,b,c){return (d3.event.pageY+15)+ "px"})
